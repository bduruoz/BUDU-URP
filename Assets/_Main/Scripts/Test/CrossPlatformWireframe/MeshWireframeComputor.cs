...
public class MeshWireframeComputor : MonoBehaviour
{
    ...
    private Color[] _SortedColoring(Mesh mesh)
    {
        ...
        {
            List<int> availableLables = new List<int>() { 1, 2, 3 };
            foreach(int vertexIndex in triangle)
            {
                if(availableLables.Contains(labels[vertexIndex]))
                    availableLables.Remove(labels[vertexIndex]);
            }
            foreach (int vertexIndex in triangle)
            {
                if(labels[vertexIndex] == 0)
                {
                    if(availableLables.Count == 0)
                    {
                        Debug.LogError("Could not find color");
                        return null;
                    }
                    labels[vertexIndex] = availableLables[0];
                    availableLables.RemoveAt(0);
                }
            }
        }
