    ...
    )
{
    ...
    for(int i = 0; i < numSteps; i++)
    {
        ...
        if( max(abs(samplePos.x), max(abs(samplePos.y), abs(samplePos.z)) ) < 0.5 + EPSILON)
        {
            if(noiseTog)
            {
                n3Da = sampleNoises((samplePos * noiseScale) + noisePos) * noiseAmp;
                n3Db = sampleNoises((samplePos * noiseScale) - (noisePos / 3)) * noiseAmp;
    ...
    //result = resultLight * color;
    //Masks = result * color.a;

    result = resultLight * 10;// * FogColor;// * color;
