...
void VoronoiPrecise3D_float(float3 UV, float AngleOffset, float CellDensity, out float3 Out, out float Cells) {
...
void Voronoi3D_float(float3 UV, float AngleOffset, float CellDensity, out float Out, out float F2_F1, out float Cells) {
    float3 g = floor(UV * CellDensity);
    float3 f = frac(UV * CellDensity);
