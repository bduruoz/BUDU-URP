...
float PrintValue(float2 vStringCoords, float fValue, float fMaxDigits, float fDecimalPlaces)
{
    ...
    float fCharBin = 0.0;
    if (fDigitIndex > (-fDecimalPlaces - 1.001))
    {
    ...
    return floor(fmod((fCharBin / pow(2.0, floor(frac(vStringCoords.x) * 4.0) + (floor(vStringCoords.y * 5.0) * 4.0))),
                      2.0));
}

float PrintValue(const in float2 fragCoord, const in float2 vPixelCoords, const in float2 vFontSize,
...
{
    res = float4(0.5, 0.2, 0.1, 1);
    res = PrintValue(uv * 200, float2(10, 120), float2(12, 15), val.x, 10, 3) * float4(1, 0, 0, 1); // red
    res += PrintValue(uv * 200, float2(10, 100), float2(12, 15), val.y, 10, 3) * float4(0, 1, 0, 1); // green
    res += PrintValue(uv * 200, float2(10, 80), float2(12, 15), val.z, 10, 3) * float4(0, 0.2, 1, 1); // blue
    res += PrintValue(uv * 200, float2(10, 60), float2(12, 15), val.w, 10, 3) * float4(1, 1, 1, 1); // white
}
