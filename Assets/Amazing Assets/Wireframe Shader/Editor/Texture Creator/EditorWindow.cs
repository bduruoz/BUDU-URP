...
namespace AmazingAssets.WireframeShader.Editor.TextureCreator
{
    internal class EditorWindow : UnityEditor.EditorWindow
    {
        ...
        [MenuItem("Window/Amazing Assets/Wireframe Shader/Texture Creator", false, 3102)]
        static public void ShowWindow()
        {
            EditorWindow window = GetWindow<EditorWindow>("Wireframe Texture Creator");
        ...
        void OnGUI()
        {
            ...
            scrollSettings = EditorGUILayout.BeginScrollView(scrollSettings);
            {
                ...
                EditorGUILayout.GetControlRect(false, 1, GUILayout.MinWidth(560));
            ...
            {
                BatchObjectsDrawer.AddBatchObjectToArray(objectPicker, true);
                SaveEditorData();
                objectPicker = null;
                Repaint();
            }
        ...
        void DrawBatchObjectsArray()
        {
            ...
            using (new EditorGUIHelper.EditorGUILayoutBeginHorizontal())
            {
                    ...
                    if (GUILayout.Button("Add Selected", GUILayout.Height(buttonHeight)))
                    {
                        for (int i = 0; i < Selection.transforms.Length; i++)
                        {
                            if (Selection.transforms[i] != null)
                                BatchObjectsDrawer.AddObjectToBatchArray(Selection.transforms[i].gameObject, false);
                        }
                ...
                if (GUILayout.Button("Add All Scene Meshes", GUILayout.Height(buttonHeight)))
                {
                    ...
                    {
                        BatchObjectsDrawer.AddObjectToBatchArray(sceneRootGameObjects[i], false);
                    }
